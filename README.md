...
brew install bash bc coreutils nowplaying-cli
```
...
```bash
apk add bash bc coreutils gawk git playerctl sed
```

#### Arch Linux

```bash
pacman -Sy bash bc coreutils git playerctl
```

#### Ubuntu

```bash
apt-get install bash bc coreutils gawk git playerctl
```
